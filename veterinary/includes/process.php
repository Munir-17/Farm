...
if (isset($_POST["manageProduct"])) {
	$m = new Manage();
	$result = $m->manageRecordWithPagination("animal",$_POST["pageno"]);
	$rows = $result["rows"];
	$pagination = $result["pagination"];
	if (count($rows) > 0) {
		$n = (($_POST["pageno"] - 1) * 5)+1;
		foreach ($rows as $row) {
			?>
				<tr>
			        ...
			        <td><a href="#" class="btn btn-success btn-sm">Active</a></td>
			        <td>
			        	<a href="#" did="<?php echo $row['productID']; ?>" class="btn btn-danger btn-sm del_product">Delete</a>
			        	<a href="#" eid="<?php echo $row['productID']; ?>" data-toggle="modal" data-target="#form_products" class="btn btn-info btn-sm edit_product">Edit</a>
			        </td>
			      </tr>
			<?php
			$n++;
...
if (isset($_POST["deleteProduct"])) {
	$m = new Manage();
	$result = $m->deleteRecord("animal","productID",$_POST["id"]);
	echo $result;
}
...
if (isset($_POST["updateProduct"])) {
	$m = new Manage();
	$result = $m->getSingleRecord("animal","productID",$_POST["id"]);
	echo json_encode($result);
	exit();
}
...
if (isset($_POST["update_product"])) {
	...
	$result = $m->update_record("animal",["productID"=>$id],["categoryID"=>$cat,"productName"=>$name,"productSprice"=>$price,"productQuantity"=>$qty]);
	echo $result;
}
...
if (isset($_POST["getNewOrderItem"])) {
	$obj = new DBOperation();
	$rows = $obj->getAllRecord("animal");
	?>
	<tr>
		    <td><b class="number">1</b></td>
		    <td>
		        <select name="productID[]" class="form-control form-control-sm productID" required>
		    ...
		    <input type="hidden" name="profit[]" type="text" class="form-control form-control-sm profit" readonly>
		    <span><input name="pro_name[]" type="hidden" class="form-control form-control-sm pro_name"></td>
		    	
		    <td>RM <span class="amt">0</span></td>
		    <td> <span hidden class="amt2">0</span></td>
	</tr>
	<?php
	exit();
}
...
if (isset($_POST["receiptDate"])) {
	...
	$orderdate = $_POST["receiptDate"];

	//Now getting array from order_form
	$ar_tqty = $_POST["tqty"];
	$ar_qty = $_POST["qty"];
	$ar_price = $_POST["price"];
    $ar_pro_Id = $_POST["productID"];
	$ar_pro_name = $_POST["pro_name"];


	$receiptSubTotal = $_POST["receiptSubTotal"];
	$netProfit = $_POST["netProfit"];

	$receiptTax = $_POST["receiptTax"];
	$receiptDiscount = $_POST["receiptDiscount"];
	$receiptNetTotal = $_POST["receiptNetTotal"];
	$paid = $_POST["paid"];
	$due = $_POST["due"];
	$receiptPayType = $_POST["receiptPayType"];
	$staffID = $_POST["staffID"];
	$memberID = $_POST["memberID"];


	$m = new Manage();
	echo $result = $m->storeCustomerOrderInvoice($orderdate,$ar_tqty,$ar_qty,$ar_price,$ar_pro_Id,$ar_pro_name,$receiptSubTotal,$receiptSubProfit,$receiptTax,$receiptDiscount,$receiptNetTotal,$paid,$due,$receiptPayType,$netProfit,$staffID,$memberID,);
