...
//code for adding product in cart
    ...
    if(!empty($_POST["quantity"])) {
      ...
      while($productByCode=mysqli_fetch_array($result)){
        $itemArray = array($productByCode["id"]=>array('catname'=>$productByCode["CategoryName"], 'quantity'=>$_POST["quantity"], 'pname'=>$productByCode["ProductName"],'image'=>$productByCode["ProductImage"], 'price'=>$productByCode["ProductPrice"],'code'=>$productByCode["id"]));
...
foreach ($value as $pdid => $qty) {
    $sql = mysqli_query($con, "SELECT * FROM tblproducts WHERE id='$pdid'");
    ...
    } elseif ($available_qty < $qty) {
        echo '<script>alert("Cannot sell more than available quantity")</script>';
    } else {
      $o=($price*$qty);
        $query = mysqli_query($con, "INSERT INTO tblorders(ProductId, Quantity, InvoiceNumber, CustomerName, CustomerContactNo, PaymentMode,AmountPaid) 
                                     VALUES ('$pdid', '$qty', '$invoiceno', '$cname', '$cmobileno', '$pmode',$o)");
        $d = mysqli_query($con, "UPDATE tblproducts SET Qty=(Qty-'$qty') WHERE id='$pdid'");
    ...
    $res = mysqli_query($con, "SELECT * FROM tblorders WHERE ProductId='$pdid'");

   /* while ($q = mysqli_fetch_assoc($res)) {
        $sql = mysqli_query($con, "SELECT * FROM tblproducts WHERE id='$pdid'");

        while ($y = mysqli_fetch_assoc($sql)) {
            $r = $y['Qty'];
 ...
 // while($q=mysqli_fetch_assoc($res)){
   ...
   /* $sql= mysqli_query($con,"select * from tblproducts where id='$pdid'");
    ...
    while($y=mysqli_fetch_assoc($sql)){
      ...
      if($quantity>$r || $r<=0){echo '<script>prompt("Cannot sell more than available quantity ")</script>';}
      /*elseif ($r<=0) { 
        //$q=mysqli_query($con,"UPDATE tblproducts set STATUS='INACTIVE' where id='$pdid'");
        echo '<script>alert("product is unavailable ")</script>'; }
      else{
        echo '<script>alert("Invoice generated successfully. Invoice number is "+"'.$invoiceno.'")</script>'; 
        $d=mysqli_query($con,"UPDATE tblproducts set Qty=(Qty-'$quantity') where id='$pdid'");
        unset($_SESSION["cart_item"]);
        $_SESSION['invoice']=$invoiceno;
        echo "<script>window.location.href='invoice.php'</script>";      
      //break ;
     ...
     /* $r=$y['Qty'];
      if($r<=0){echo 'product is unavailable';}
      else{
        $prodRemaing=$r-$t;
        $sql2= mysqli_query($con,"UPDATE tblproducts set Qty='$prodRemaing' where id='$pdid'");
        echo '<script>alert("Invoice generated successfully. Invoice number is "+"'.$invoiceno.'")</script>'; 
        unset($_SESSION["cart_item"]);
        $_SESSION['invoice']=$invoiceno;
        echo "<script>window.location.href='invoice.php'</script>";
    ...
    //$upt=mysqli_query($con,"UPDATE tblproducts set Qty='$prodRemaing' where id=$pid");*/
    //break;
    //$d=mysqli_query($con,"UPDATE tblproducts set Qty=(Qty-'$quantity') where id='$pdid'");
  }
                   ...
                   if(isset($_SESSION["cart_item"])){
                    ...
                    <div class="form-row">
                      ...
                      <div class="col-md-6 mb-10">
                        ...
                        <input type="number" pattern="^[0-9,]+$" class="form-control" id="validationCustom03" placeholder="Mobile Number" name="mobileno" required>
                    ...
                    <div class="form-row">
                      ...
                      <div class="col-md-6 mb-4 ">
                        <button class="btn btn-primary mt-6" type="submit" name="checkout">Purchase</button>
                      </div>
                    </div>
                  </form>
                  <?php
