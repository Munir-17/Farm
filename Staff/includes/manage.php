...
class Manage
{
	...
	public function manageRecordWithPagination($table,$pno){
		...
		if ($table == "categories") {
			...
			$sql = "SELECT p.productID,p.product_name,c.category_name,b.brand_name,p.product_price,p.product_stock,p.added_date,p.p_status FROM products p,brands b,categories c WHERE p.bid = b.bid AND p.cid = c.cid ".$a["limit"];
		}else{
...
//---------------------------START-----------------------------
	public function storeCustomerOrderInvoice($orderdate,$ar_tqty,$ar_qty,$ar_price,$ar_pro_Id,$ar_pro_name,$receiptSubTotal,$receiptSubProfit,$receiptTax,$receiptDiscount,$receiptNetTotal,$paid,$due,$receiptPayType,$netProfit,$staffID,$memberID){
		$pre_stmt = $this->con->prepare("INSERT INTO 
			`receipt`(`receiptDate`, `receiptSubTotal`,
			 `receiptTax`, `receiptDiscount`, `receiptNetTotal`,`receiptPaid`,`receiptBalance`,`receiptPayType`,`receiptProfit`, `staffID`, `memberID`) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
				        
				if($memberID == '') $memberID = null;


		$pre_stmt->bind_param("sddddddssss",$orderdate,$receiptSubTotal,$receiptTax,$receiptDiscount,$receiptNetTotal,$paid,$due,$receiptPayType,$netProfit,$staffID,$memberID);
		$pre_stmt->execute() or die($this->con->error);
		...
		if ($invoice_no != null) {
			for ($i=0; $i < count($ar_price) ; $i++) {
				...
				if ($rem_qty < 0) {
					...
					$sql = "UPDATE animal SET productQuantity = '$rem_qty' WHERE productName = '".$ar_pro_name[$i]."'";
					$this->con->query($sql);
				}


				$insert_product = $this->con->prepare("INSERT INTO `receipt_line`(`receiptID`, `productID`,`productName`, `price`, `quantity`)
				 VALUES (?,?,?,?,?)");
				$insert_product->bind_param("iisdd",$invoice_no,$ar_pro_Id[$i],$ar_pro_name[$i],$ar_price[$i],$ar_qty[$i]);
